...
var tempShiftOrigin = null
var tempShiftedGemTween;
var allowInput;

function preload() {

game.load.spritesheet("GEMS", "diamonds32x5.png", GEM_SIZE, GEM_SIZE);
...
_matchedGems = matchedGems();

// if (tempShiftedGem !== null)
// {
//     matchedGems(tempShiftedGem);
// }
...
if (selectedGem && pointer.isDown && !fromClick)
{
var cursorGemPosX = getGemPos(x);
var cursorGemPosY = getGemPos(y);
if(checkIfGemCanBeMovedHere(selectedGem.posX, selectedGem.posY, cursorGemPosX, cursorGemPosY))
{
    gems.bringToTop(selectedGem); // bring selectedGem to top
    tempShiftedGem = getGem(cursorGemPosX, cursorGemPosY); // Get under-gem
    selectedGemTween = tweenGemPos(selectedGem, cursorGemPosX, cursorGemPosY); // selected slides over under-gem
    underGemTween = tweenGemPos(tempShiftedGem, selectedGem.posX , selectedGem.posY);
...
function countSameColorGems(startGem, moveX, moveY) {
  ...
  var curX = startGem.x + moveX;
  var curY = startGem.y + moveY;
  var count = 0;

  // while (curX >= 0 && curY >= 0 && curX < BOARD_COLS && curY < BOARD_ROWS && getGemColor(getGem(curX, curY)) === getGemColor(startGem))
  while (findGem(curX,curY) && findGem(curX,curY).c === startGem.c)
  {
    count++;
    curX += moveX;
    curY += moveY;
...
function matchedGems() {
  ...
  for(var r=0; r<BOARD_ROWS;r++) {
    for(var c=0; c<BOARD_COLS;c++) {
      ...
      //[0,0,0,3]
      visibleRuns(_gem).forEach(function(_runLength){
        if(_runLength > 2) return;
        var _run_direction = (_i % 2 === 0 ? 'vertical' : 'horizontal');
        var _run = [_gem];
        //---
        //---
...
function visibleRuns(gem, direction){
    // TODO optomize to only call when requested
